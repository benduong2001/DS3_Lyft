...
class Input_Data_Table_Executor:
    ...
    def iterate_cluster_sequences_partitions(self, list_frame_clusters):
        if None in list_frame_clusters: # if there is a hole in the segment, the whole thing is broke; ignore.
            return None
    ...
    def iterate_frames(self, agent_table_scene_selection, frame_indices):
        ...
        for i in range(len(frame_indices) - RANK):
            ...
            if partitions == None:
                ...
                cluster_sequence_partitions += partitions
                
        return cluster_sequence_partitions
            
    def iterate_scenes(self,
                       scene_list_indices = None,
                       frame_list_index_start = ((248//8) * 0),
                       frame_list_index_end   = ((248//8) * 8)):
        #exa1
        ...
        for i in scene_list_indices:
            ...
            if partitions == None:
                ...
                scene_partitions.append(partitions)
##        
...
pickle_file_name = folderpath + "lyft_scripts\\lyft_partitions.pkl"
def create_file():
...
##    scene_partitions_dataset = manager.list()
    ...
    def session(session_int, scene_partitions_dataset):
...
##        worker_pool.map(multiproc_serialize_dataset, args_iter)
    ...
    for i in range(0, 10):
        ...
        session(i, scene_partitions_dataset)
...
##        f.close()
##        print("\t curr stored_data is len {0}".format(len(stored_data)))
##        print("\t curr gained data is len {0}".format(len(scene_partitions_dataset)))
##        
##        new_stored_data = [x for x in scene_partitions_dataset]
##        f = open(pickle_file_name, 'wb') 
##        pickle.dump(new_stored_data, f)          # dump data to f
##        f.close()
##        print("OVERWRITING FILE DONE; YOU MAY NOW HALT")
        print(len(scene_partitions_dataset))

        temp_filename = folderpath + '\\lyft_scripts\\lyft_partitions_{0}_{1}.pkl'.format(i+1, SESSIONS)
        f = open(temp_filename, 'wb')
        cPickle.dump(list(scene_partitions_dataset) , f)
        f.close()
