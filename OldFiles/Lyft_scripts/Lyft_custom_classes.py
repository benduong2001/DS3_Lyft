...
NUMERICAL_FEATURES = ["centroid_x", "centroid_y", "yaw", "speed", "mean_area"] # <- Remove as many as you want
...
##    def _standardizations3_zeropad(self, pivot_table_arr):
##        # Only possible after numpy_conversion
##        pivot_table_arr = np.nan_to_num(pivot_table_arr)
##        return pivot_table_arr
##
##    def _standardizations2a_dimensions_frame(self, rotated_pre_pivoted_agent_table_frame):
##        """
##        table of 1 frame in the 3 batch; we will individually pad/cap them to be 20 rows
##        remember that at this stage, the features are still across, so we only need to pad/cap once
##        but we will have to use dummy track_ids
##
##        """
##
##        global MAX_TRACK_IDS
##        global PIVOTED_FEATURES
##        PIVOTED_FEATURES_AMOUNT = len(PIVOTED_FEATURES)
##        
##        place_holder = 0
##        table_height = len(rotated_pre_pivoted_agent_table_frame)
##        
##        return rotated_pre_pivoted_agent_table_frame_standard_height 
##        
##
##
##    def _standardizations2_dimensions(self, rotated_pre_pivoted_agent_table):
##        # Only possible after numpy conversion
##        global MAX_TRACK_IDS
##        global PIVOTED_FEATURES
##        PIVOTED_FEATURES_AMOUNT = len(PIVOTED_FEATURES)
##        rank = len(pd.unique(rotated_pre_pivoted_agent_table["frame_index"])) # is 3
##
##        counts = rotated_pre_pivoted_agent_table.groupby(['frame_index'])['frame_index'].size().values.astype(int)
...
##        place_holder = 0
##        
##        num_track_ids = pivot_table_arr.shape[1] // PIVOTED_FEATURES_AMOUNT
##        track_ids_disparity = MAX_TRACK_IDS - num_track_ids
##            
##        if track_ids_disparity < 0:
##            # oversized
##            pivot_table_arr_standardized = pivot_table_arr[:,:MAX_TRACK_IDS * PIVOTED_FEATURES_AMOUNT]
##        else:
##            dummy_array = np.full((rank, track_ids_disparity * PIVOTED_FEATURES_AMOUNT), place_holder)
##            try:
##                pivot_table_arr_standardized = np.column_stack((pivot_table_arr, dummy_array))
##            except:
##                print(pivot_table_arr.shape, dummy_array.shape)
##                assert False
##        return pivot_table_arr_standardized
